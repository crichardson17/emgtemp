...
Cloudy_File = '/Users/Sam/Documents/emgtemp/Studies/Max_Temp_Studies/max_temp_sim.pun'
...
Grid_File = '/Users/Sam/Documents/emgtemp/Studies/Max_Temp_Studies/max_temp_sim.grd'
...
grains_colors = [plt.cm.Reds(i) for i in np.linspace(0.25,1,7)]
u_colors = [plt.cm.Purples(i) for i in np.linspace(0.25,1,6)]
...
sm = plt.cm.ScalarMappable(norm=colors.Normalize(vmin=0.25, vmax=7.0),cmap=grains_colors_map)
sm._A = []
...
sm = plt.cm.ScalarMappable(norm=colors.Normalize(vmin=0.25, vmax=3.0),cmap=u_colors_map)
sm._A = []
...
plt.savefig("Z_0.5_2.0_Sims_Plots.png", dpi = 600)
